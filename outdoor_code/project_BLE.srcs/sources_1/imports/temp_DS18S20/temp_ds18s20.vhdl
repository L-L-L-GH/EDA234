...
architecture rtl of ds18s20_onewire is
    ...
    constant T_US     : integer := CLK_FREQ / 1_000_000; -- 100 ticks per us

    -- 1-Wire Standard Timing
    constant T_RESET  : integer := 480 * T_US; -- Reset Pulse
    constant T_WAIT   : integer := 480 * T_US; -- Wait Presence
    
    constant T_SLOT   : integer := 70 * T_US;  -- Total Slot duration (incl recovery)
    ...
    constant T_CONV   : integer := 750_000 * T_US; -- 750ms conversion
        ...
        if rising_edge(clk) then
            if rst_n = '0' then
                ...
                case state is
                    ...
                    -- 2. Wait Presence (Blind wait is okay for now, but checking dq_in is better)
                    ...
                    -- 3. Write Skip ROM (0xCC)
                    when WRITE_SKIP_ROM_1 =>
                        -- Decision logic for driving the bus
                    ...
                    when WAIT_CONVERSION =>
                        -- Strong pullup could be applied here for parasite power, 
                        -- but for external power just wait.
                        if cnt = T_CONV then
                            cnt   <= 0;
                            state <= RESET_2;
                        else
